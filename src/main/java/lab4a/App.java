...
public class App {
  public static class AppMapper extends Mapper<LongWritable, Text, Text, IntWritable> {
    public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
      ...
      String skey = tokens.length > 4 ? tokens[tokens.length - 1] : tokens[3];

      float price = Float.parseFloat(sPrice);
      context.write(new Text(skey), new IntWritable((int) price));
    }
  }
  ...
  public static class AppReducer extends Reducer<Text, IntWritable, Text, IntWritable> {
    public void reduce(Text key, Iterable<IntWritable> values, Context context)
      ...
      for (IntWritable value : values) {
        // Kiem tra mat hang co gia lon hon 100000
        if (value.get() > 150000) {
          count++;
        }
      }
